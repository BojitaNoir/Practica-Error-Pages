...
class CustomUserCreationForm(UserCreationForm):
    ...
    def clean_password1(self):
        ...
        if not re.match(r'^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[!@#$%&?])[A-Za-z\d!@#$%&?]{8,}$', password):
            raise forms.ValidationError("La contraseña debe tener al menos 8 caracteres, una mayúscula, una minúscula, un número y un símbolo especial (!@#$%&?).")
        return password
    
    def clean_control_number(self):
        control_number = self.cleaned_data.get("control_number")
        if not re.match(r'^[A-Za-z0-9]{10}$', control_number):
...
class CustomUserLoginForm(AuthenticationForm):
    def clean(self):
        ...
        if email and password:
            user = authenticate(email=email, password=password)  # Cambia 'username' por 'email'
            if not user:
